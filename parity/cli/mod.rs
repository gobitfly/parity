...
usage! {
	...
	}
	{
		...
		["Sealing/Mining Options"]
			...
			FLAG flag_tx_queue_no_cheap_checks: (bool) = false, or |c: &Config| c.mining.as_ref()?.tx_queue_no_cheap_checks.clone(),
			"--tx-queue-no-cheap-checks",
			"Skipts the cheap checks when importing transactions into the tx queue. This will allow transactions from local accounts that were received via the P2P layer to enter the tx queue independent of their gas price.",
...
struct Mining {
	...
	tx_queue_no_cheap_checks: Option<bool>,
